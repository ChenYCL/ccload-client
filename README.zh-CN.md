...
跟版是自动的：`.github/workflows/kernel-sync.yml` 每小时看一眼上游最新 release（含 prerelease，内核发的正是 beta），不一样就跟上、提交、派发一次 beta 打包。**提交之前先把内核编一遍** —— 内核偶尔会引入新的构建前置，编不出来就该红在那条流水线上，而不是把一个编不出来的钉子推上 main。
