        ...
        <div className="scroll-edge flex-1 overflow-auto px-4 py-3">
          {log.message && (
            <section className="mb-4">
              <h3 className="mb-1 text-[11px] text-muted">{t("上游返回")}</h3>
              <pre
                className={cn(
          ...
          <Group title={t("路由")}>
            <Row k={t("渠道")} v={log.channel_name ?? (log.channel_id ? `#${log.channel_id}` : "—")} />
            <Row k={t("实际模型")} v={log.actual_model ?? log.model ?? "—"} mono />
            <Row k={t("上游地址")} v={log.base_url ?? "—"} mono wrap />
            <Row k={t("客户端协议")} v={log.client_protocol ?? "—"} />
            <Row k={t("上游协议")} v={log.upstream_protocol ?? "—"} />
            <Row k={t("流式")} v={log.is_streaming ? t("是") : t("否")} />
            {log.thinking_effort && <Row k={t("思考强度")} v={log.thinking_effort} />}
          </Group>

          <Group title={t("耗时")}>
            <Row k={t("总耗时")} v={fmtDuration(log.duration)} />
            <Row k={t("首字节")} v={fmtDuration(log.first_byte_time)} />
          </Group>

          <Group title="Tokens">
            <Row k={t("输入")} v={fmtInt(log.input_tokens)} />
            <Row k={t("输出")} v={fmtInt(log.output_tokens)} />
            {log.reasoning_tokens != null && (
              <Row k={t("推理")} v={fmtInt(log.reasoning_tokens)} />
            )}
            <Row k={t("缓存读取")} v={fmtInt(log.cache_read_input_tokens)} />
            <Row k={t("缓存写入")} v={fmtInt(log.cache_creation_input_tokens)} />
          </Group>

          <Group title={t("费用与来源")}>
            {/* logs.cost 是标准成本，实付要乘渠道倍率 —— 见 effectiveCost。 */}
            <Row k={t("费用（倍率后）")} v={fmtCost(effectiveCost(log))} />
            {log.cost_multiplier != null && log.cost_multiplier !== 1 && (
              <>
                <Row k={t("标准成本")} v={fmtCost(log.cost)} />
